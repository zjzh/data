...
#        print("word_list_1: ",word_list)
        ...
        for index,swift_words in enumerate(all_swift_words):
            ...
            if  set(swift_words)>set(word_list):
                ...
                sim=1 - sp.distance.cdist([j_weight_list], [swift_weight[index]], 'cosine')
...
#        weight_list=list(map(lambda x: swift_weight[i][x],api))
    ...
    for i,value in enumerate(map_java_list_name):
        ...
        dst_api_sig_list=extract_keywords(top_k,np.array(map_java_to_swift),s_new_weight_list,word,src_api_sig_list,all_api_sig_list[api_j_num:],all_swift_words)
